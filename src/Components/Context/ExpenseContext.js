...
export const ExpenseContextProvider = (props) => {
  ...
  const deleteExpenseHandler = (id) => {
    const deleteExpenseItem = async (id) => {
      try {
        const response = await fetch(
          `https://expense-tracker-b4081-default-rtdb.firebaseio.com/expenses/${id}.json`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const editExpenseHandler = (expenseItem) => {
    const editExpenseItem = async (expenseItem) => {
      try {
        const response = await fetch(
          `https://expense-tracker-b4081-default-rtdb.firebaseio.com/expenses/${expenseItem.id}.json`,
          {
            method: "PUT",
            body: JSON.stringify({
